defmodule IO.ANSI.Table.StyleTest do
  ...
  describe "Style.borders/2" do
    test "returns the borders of a table style and line type" do
      ...
      expected2 = ["╟─", "─┼─", "─╢"]
      expected3 = []
      assert Style.borders(:light, :bottom) == expected1
      assert Style.borders(:medium, :separator) == expected2
      assert Style.borders(:heavy, :bottom) == expected3
    end
  end
